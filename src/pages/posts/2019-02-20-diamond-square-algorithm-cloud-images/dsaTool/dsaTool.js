...
export default class DSATool extends React.Component {
  ...
  runDSA(N){
    ...
    while(side > 1){
      ...
      let xc,yc;
      // Diamond step:
      this.diamondStep(xc, yc, arr, nSq, side, sideo2, f);
      //Square step:
      this.squareStep(xc, yc, arr, nSq, side, sideo2, f, N);
      side = sideo2;
      nSq = nSq*2;
      f = f/2;
    }
  ...
  //Print array to screen:
	printToCanvas(arr){
    this.canvas.current.width = arr.length;
    this.canvas.current.height = arr.length;
    const ctx = this.canvas.current.getContext("2d");
    
		const myImageData = ctx.getImageData(0,0, arr.length, arr.length);
		const rgb = this.hexToRgb(this.state.color1);
